...
const AlbumsDetailScreen({ Key? key }) : super(key: key);
        ...
        children: [
          GlassContainer(
            ...
            child: Row(
              children: [
                QueryArtworkWidget(
                  id: ref.watch(currentAlbumStateProviderProvider).id, 
                ...
                Expanded(
                  child: Padding(
                    ...
                    child: Column(
                      ...
                      children: [
                        Text(ref.watch(currentAlbumStateProviderProvider).album, maxLines: 1, overflow: TextOverflow.clip,),
                        Text(ref.watch(currentAlbumStateProviderProvider).artist, maxLines: 1, overflow: TextOverflow.clip,),
                        Text('Track: ${ref.watch(currentAlbumStateProviderProvider).numOfSongs}'),
          ...
          Expanded(
            child: ValueListenableBuilder<List<SongModel>>(
              valueListenable: audioManager.songsAlbumListNotifier,
              builder: (_, songList,__) {
                return ListView.builder(
                  itemCount: ref.watch(currentAlbumStateProviderProvider).numOfSongs,
                  itemBuilder: (context, index) {
                    return ListItem(
                      title: Text(songList[index].title, maxLines: 1,),
                      subtitle: Text(songList[index].artist ?? "", maxLines: 1,),
